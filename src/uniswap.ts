...
const provider = new ethers.providers.JsonRpcProvider('https://polygon-mainnet.infura.io/v3/7404c4bb8f6b40de9b3fd1e973033eee');
console.log('uniswap.ts - Ethereum provider initialized');
...
console.log('uniswap.ts - Tokens initialized: USDC and EURO3');

// Uniswap Pool Address
const poolAddress = '0xE1f9709D32Db8A79Ae44F66299e1a93cA84DEbE3';
console.log(`uniswap.ts - Uniswap Pool Address: ${poolAddress}`);


// Initialize Pool Contract
const poolContract = new ethers.Contract(poolAddress, UniswapV3PoolABI, provider);
console.log('uniswap.ts - Uniswap pool contract initialized');


async function getPoolData() {
    console.log('uniswap.ts - Fetching pool data...');
    const [liquidity, slot] = await Promise.all([
        poolContract.liquidity(),
        poolContract.slot0(),
    ]);

    console.log('uniswap.ts - Pool data fetched successfully');
...
async function calculateCurrentPrice() {
    console.log('uniswap.ts - Calculating current price...');
    try {
        const pool = await getPoolData();
        const price = pool.token0Price;
        const formattedPrice = price.toFixed(6);
        console.log(`uniswap.ts - Current EURO3 Price: ${formattedPrice}`);
        ...
        console.error("uniswap.ts - Error in calculating price: ", error);
...
async function monitorAndManagePeg() {
    console.log('uniswap.ts - Starting to monitor and manage peg...');
    ...
    while (true) {
        try {
            ...
            if (price.lessThan(pegPrice.subtract(pegPrice.multiply(threshold)))) {
                        console.log('uniswap.ts - EURO3 is under-peg. Buying EURO3...');
                        ...
                        console.log('uniswap.ts - EURO3 is over-peg. Selling EURO3...');
                        botData.lastAction = 'Selling EURO3';
                        // Implement your selling logic here
                    }
                } catch (error) {
                    console.error("uniswap.ts - Error in monitoring: ", error);
